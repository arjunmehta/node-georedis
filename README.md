...
First argument can either be an object with `latitude` and `longitude`, or a `locationName` as a **String**. Returns an `Array` of either `locationName`s or `locations`.
