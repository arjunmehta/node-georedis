...
EmulatedInterface.prototype.nearby = function(point, distance, options, zSetName, callBack) {
    ...
    var withValues = (precise || options.withDistances || options.withCoordinates || options.withHashes || options.order) ? true : false;

    distance = convertUnitsToMeters(units, distance);

    ranges = range(point.latitude, point.longitude, distance, precise);
