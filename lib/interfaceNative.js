...
function georadiusGeneric(client, type, argArray, radius, options, callBack) {
    ...
    if (options.order) {
        argArray.push(options.order);
    }

    client.send_command(type, argArray, function(err, results) {

        if (err) {
            callBack(err, null);
        } else {
            if (withCoordinates || withDistances || withHashes) {
                locations = processLocations(results, withCoordinates, withDistances, withHashes, units);
                callBack(null, locations);
            } else {
                Object.defineProperty(results, 'locationSet', {
                    get: function() {
